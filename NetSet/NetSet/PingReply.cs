...
namespace NetSet
{
    ...
    public class PingReply2
    {
        ...
        internal PingReply2(uint nativeCode, int replystatus, IPAddress ipAddress, TimeSpan duration)
        {
        ...
        internal PingReply2(uint nativeCode, int replystatus, IPAddress ipAddress, int roundTripTime, byte[] buffer)
        {
            _nativeCode = nativeCode;
            _ipAddress = ipAddress;
        ...
        public static PingReply2 Send(IPAddress srcAddress, IPAddress destAddress, int timeout = 5000, byte[] buffer = null, PingOptions po = null)
        {
            ...
            try
            {
                ...
                if (nativeCode == 0) //Means that native method is faulted.
                    return new PingReply2(nativeCode, reply.Status, new IPAddress(reply.Address), duration);
                else
                    return new PingReply2(nativeCode, reply.Status, new IPAddress(reply.Address), reply.RoundTripTime, replyBuffer);
            }
            finally
            {
